...
module CommandExecute {
    ...
    export function customCommand_1(commandPage: CommandPage, cmd: Command, trigger: CommandTrigger, triggerPlayer: ClientPlayer, playerInput: any[], p: CustomCommandParams_1): void {
        ...
        // 战斗资源
        let sceneArr = [];
        let battleAssetArr = g(8);
        if (battleAssetArr.length > 0) {
            if (GameBattle.setting) {
                let enemyParty: Module_Party = GameData.getModuleData(11, GameBattle.setting.enemyParty);
                if (enemyParty) {
                    if (enemyParty.battlerBgType == 0) imageArr.push(enemyParty.background);
                    else if (enemyParty.battleScene > 0) sceneArr.push(enemyParty.battleScene);
                    for (let i = 0; i < enemyParty.enemys.length; i++) {
                        let actor: Module_Actor = GameData.getModuleData(6, enemyParty.enemys[i].actor);
                        if (actor) avatarArr.push(actor.bttlerAvatar);
                    }
                }
